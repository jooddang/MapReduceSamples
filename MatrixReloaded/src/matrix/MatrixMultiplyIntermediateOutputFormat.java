...
public class MatrixMultiplyIntermediateOutputFormat extends FileOutputFormat<PositionWritable, IntWritable> {
	...
	public RecordWriter<PositionWritable, IntWritable> getRecordWriter(
			...
			throws IOException {
		// We don't use compression for intermediate output.
	...
	protected static class MatrixIntermediateRecordWriter implements RecordWriter<PositionWritable, IntWritable> {
		...
		public void write(PositionWritable key, IntWritable value)
			...
			out.write(Integer.toString(key.getFinalRow()).getBytes(utf8));
			out.write(separator.getBytes(utf8));
			out.write(Integer.toString(key.getFinalCol()).getBytes(utf8));
			out.write(separator.getBytes(utf8));
			out.write(((key.getInputName() == null) ? " " : key.getInputName()).getBytes(utf8));
			out.write(separator.getBytes(utf8));
