...
public class MatrixMultiplier extends Configured implements Tool {
	...
	public static class MultiplyMapper extends MapReduceBase implements Mapper<PositionWritable, IntVectorWritable, PositionWritable, IntWritable> {
		...
		private int AColNum, BColNum, ARowNum, BRowNum;
			...
			if (currentFile.endsWith(firstInput)) {
				...
				while (value.hasNext()) {
					...
					for (int i = 0; i < BColNum; i++) {
						interRow = key.getRow() * BColNum + i; // AColNum == BRowNum
						interCol = colIndex;
				...
				System.out.println("Total output : " + loopCounter);
			}
		...
		public void configure(JobConf conf) {
			...
			BRowNum = conf.getInt("BRowNum", 0);
			AColNum = conf.getInt("AColNum", 0);
...
//				"/user/daybreaker/matrices/matrix-3x2-0");
		FileInputFormat.setInputPaths(conf, "/intern/matrices/matrix-2x4-0,/intern/matrices/matrix-4x7-0");
		conf.set("First", "matrix-2x4-0");
		conf.set("Second", "matrix-4x7-0");
		conf.setInt("ARowNum", 2);
		conf.setInt("AColNum", 4);
		conf.setInt("BRowNum", 4);
		conf.setInt("BColNum", 7);
